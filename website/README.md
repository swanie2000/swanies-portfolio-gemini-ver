...
3. **`index.html` — Get the app** — Public Play CTA when **`PLAY_URL`** is set. **Invite-only** closed-test QR (**`TESTER_URL`** — Closed testing → Testers → Copy link: **`apps/testing/com.swanie.portfolio`**). Gmail must be on Closed Alpha email list.
