...
3. **`index.html` — Get the app** — Public Play CTA when **`PLAY_URL`** is set. **Invite-only** internal-test QR (**`TESTER_URL`** — Internal testing → Testers → Copy link) for family; closed testing is not on the site (Testers Community uses Play Console directly).
