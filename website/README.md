...
3. **`index.html` — Get the app** — Production **`#get-app`** section: Google Play CTA (set **`PLAY_URL`** when the listing is live), **Play internal test** QR + button (**`TESTER_URL`** — Play Console → Internal testing → Testers → **Copy link**), **contact form** at **`contact.html`** (Web3Forms), and a **site QR** for **`SITE_SHARE_URL`**. QR codes use **`website/js/qrcode.min.js`** (MIT — **`website/js/NOTICE-qrcodejs.txt`**).
