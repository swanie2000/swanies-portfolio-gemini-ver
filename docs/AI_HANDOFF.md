...
**Last updated:** 2026-05-16 — **Owner pause (stepping away):** Owner will pick up **Play listing**, **Pages deploy check**, and **device install QA** when ready; **no code blockers** in repo. **GitHub `main`:** includes **Internal testing** release **2 (1.0.1)** for **`com.swanie.portfolio`** (signed AAB path); tester list **Swanie's Portfolio Testers**; **`website/index.html`** — **`TESTER_URL`** internal opt-in, **`TESTER_REQUEST_EMAIL`** + **mailto** “request access” form + copy that **manual add in Console** is required before the Play link works; **`PLAY_URL`** still empty until public listing. **Android:** **`versionCode` 2** / **`versionName` 1.0.1**; **`swan_asset_toast.png` → `swan_asset_toast.jpg`** (JPEG bytes under `.png` broke **`mergeReleaseResources`**). Prior: **metal/widget** headlines (**`metalWidgetHeadlinePair`**), **`PortfolioWidget`**, **`__METAL_DEFAULT__`** pack order; **laptop-only** dev (**`.cursor/rules/git-pull-first.mdc`**). **`:app:bundleRelease`** green after toast fix.
...
- **Testing path:** **Internal testing** has live release **2 (1.0.1)** + tester email list; opt-in link wired in **`website/index.html`** (`TESTER_URL`). Confirm **GitHub Actions** deployed **`website/`** to **`https://swaniedesigns.com`**. Install from Play on a device (same Google account as list) for QA. **Closed testing** when ready for **≥12** opted-in testers × **14** days toward production eligibility.
...
1. **Bump `versionCode`** in `app/build.gradle.kts` before each **new** Play upload (Play rejects duplicate codes). *(Done for **2** / **1.0.1** — next build needs **3+**.)*
2. **Android Studio:** **Build → Generate Signed App Bundle** → **release** AAB with **`swanie_portfolio_release.jks`**. *(Done for internal **2**.)*
3. **Play Console → Internal testing:** *(Release **2 (1.0.1)** rolled out; testers list saved.)* For future builds: **Create new release** → upload AAB → notes → **Save** → **Review** → **Roll out**.
...
1. **Play (human) — digest & listing:** Owner stepping back to read Console; when ready continue **default store listing** (screenshots, icon, feature graphic, content rating, target audience, privacy URL, ads declaration, short description). **Internal testing** is live (**2 / 1.0.1**); next Play upload needs **`versionCode` ≥ 3**. **Publishing overview → Send for review** when Dashboard green. **`Master_Build_Checklist.md`** in sync when milestones tick.
2. **Verify cloud surfaces:** Confirm **GitHub Actions** deployed latest **`website/`** to **`https://swaniedesigns.com`** (`#join-testing`, tester form, **`TESTER_URL`** QR). Install **Swanie's Portfolio** from **Play** on a device using a listed tester account.
3. **i18n (maintenance):** **Privacy & Terms §1–§6** synced across **`values-*`** (2026-05-12). When **`values/strings.xml`** changes again, propagate keys; spot-check **About** before listing freeze. **`website/`** deploy is separate from app strings.
4. **Website:** Set **`PLAY_URL`** in **`website/index.html`** when the **public** Play listing URL exists (`TESTER_URL` + **`TESTER_REQUEST_EMAIL`** already set for internal path).
5. **Optional cleanup:** **`app/src/main/assets/adi-registration.properties`** (ADI); **`.idea`** **`assetWizardSettings.xml`** untracked — do not re-add without cause.
6. **Pre-launch QA:** Backup round-trip; purchase / restore / expiry; **widgets** (Pro **8**-row) + Pro gates; GRAM/KILO metals on device (Play-delivered build).
7. **Backlog (post-v1):** V40.36 auth instrumentation, V40.61 monetization telemetry, V40.69 small-screen polish — only if scheduled.
8. **Icons (optional / post-v1):** **`swan_launcher_extra_small_hq.xml`** scale/translate; splash/toast/widget wrappers.
...
- **Widget (Glance):** Pro **8** / free **3** holdings rows; per-line preference packing + pipe-tolerant parse; list column nests each row (**RemoteViews** direct-child limit); metal titles use **`metalWidgetHeadlinePair`** on widget + in-app cards; **`__METAL_DEFAULT__`** / **`file:`** icon packing on push.
...
| Home screen widget | **`PortfolioWidget.kt`** (Glance; **`WidgetAssetLimits`** Pro **8** / free **3**; **`writeWidgetPackedAssetRows`** per-line prefs; **`parseSingleWidgetAssetEntry`** pipe-safe; **nested `Column` per row** for RemoteViews child cap); **`AssetRepository.kt`**, **`SettingsViewModel.kt`**; **`WidgetManagerScreen.kt`**, **`AssetViewModel.kt`**, **`WidgetConfigActivity.kt`** — metal titles: **`metalCardPrimaryLabel`** + **`metalWidgetHeadlinePair`** (widget + collapsed/expanded; run-together **`SILVERBAR`**); packed **`iconSource`** uses **`__METAL_DEFAULT__`** / **`file:`** order (see **§ Current session**) |
...
| Marketing site | `website/` — **`index.html`** (**`PLAY_URL`** / **`TESTER_URL`** / **`TESTER_REQUEST_EMAIL`** join-testing mailto form, **QR** **`drawQr`** + debounced **`resize`**, **`.shots-carousel`** + **arrows-below** nav), **`privacy.html`** (**§4** **`#account-deletion`**, **`#data-deletion`**), **`styles.css`**, **`README.md`**, **`ic_swan_website.png`**, **`favicon-tab.png`**, **`images/*.jpg`**, **`js/qrcode.min.js`**, **`play_store_app_icon_512.png`** / **`play_store_feature_graphic_1024x500.png`**, **`scripts/compose-play-feature-graphic.ps1`**, `.github/workflows/deploy-website.yml` |
