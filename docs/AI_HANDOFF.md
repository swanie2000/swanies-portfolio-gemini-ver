...
**Last updated:** 2026-06-02 — **Widget Pro fix shipped in repo as 1.0.24** (not on Play yet). **1.0.23 bug:** app auto-Pro OK, widget still **free tier** (3 rows, upgrade banner) — **`AssetRepository`** / **`WidgetAssetLimits`** only checked RevenueCat. **Fix:** **`WidgetAssetLimits.isProForWidget`** + **`ClosedTestProAccess`**; widget refresh on Pro change in **`SettingsViewModel`**. **Still waiting on Google review** (closed Alpha). **Next:** build signed **24** → **`verify-play-release.ps1`** → upload **internal** + **closed** (replace **23**).
...
| **Play internal testing** | **Active: 23 (1.0.23)** — auto-Pro until **~2026-07-01** (build-time cutoff). Family on **Internal testing → Testers** (list 1); **`swaniedesigns.com/#get-app`** internal invite QR. |
| **Play closed testing (Alpha)** | **Active: Closed Test 23 (1.0.23)** — **Google Groups** **`testers-community@googlegroups.com`**; feedback **`contact.html?topic=tester`**. TC opt-in link submitted. **≥12** opted-in **14+** consecutive days for **production access** — **1** opted-in (owner); **Google review** pending (owner). |
| **Version on device** | Play **23 (1.0.23)** — app Pro OK; **widget Pro broken** on **23** (owner). Repo **24 (1.0.24)** fixes widget; **upload pending**. |
...
- **Testing path:** **Closed Alpha** live — TC **opt-in URL** submitted (**25** paid on testerscommunity.com). **≥12 opted-in for 14 consecutive days** → **Production access** (owner **1** opted-in; **waiting on Google review**). **License testing** **unchecked** for TC. **Site:** **`#get-app`** = **invite-only internal** QR only (**`TESTER_URL`**); closed link **not** on site (TC gets Play opt-in directly).
- **Marketing site (2026-06-01):** **`contact.html`** — **Tester feedback** topic + **`?topic=tester`** preselect (**`4f6197f`**); Play **Feedback URL** aligned. **`#get-app`** — internal invite QR visible by default (**`d64e728`**).
...
**Play / Google:** **Internal** + **Closed Alpha** **23 (1.0.23)** live; **auto-Pro** QA passed. **License testing** **unchecked**. **Production access:** **waiting on Google review** + **≥12** closed testers **14+** days. **Production** release not shipped.
...
1. **Upload 1.0.24 (24)** — **`verify-release-config.ps1`** → Signed Bundle → **`verify-play-release.ps1`** → promote to **internal** + **closed Alpha** (replace **23**).
2. **Phone QA on 24:** App Pro + **widget** (8 rows, no upgrade banner, Widget Manager) with **`GRANT_DAYS=30`** and no RC entitlement.
3. **Wait / monitor Google review** — unblock **production access** or other Console gates as Dashboard shows.
4. **Closed-test gate:** Grow **≥12** opted-in testers on **Closed Alpha** for **14 consecutive days**.
5. **Production release:** **`CLOSED_TEST_PRO_GRANT_DAYS=0`**, bump version, verify, **Send app for review** → staged rollout.
6. **At public listing:** Set **`PLAY_URL`** in **`website/index.html`** → push **`main`**.
...
1. **Ship 1.0.24** — widget auto-Pro fix; replace **23** on internal + closed tracks after phone QA.
2. **Google review** — owner **waiting**; recheck **Dashboard** when status changes.
3. **Closed-test metrics:** **≥12** opted-in, **14+** consecutive days on **Closed Alpha**.
4. **Production release:** **`GRANT_DAYS=0`** build → **`verify-play-release.ps1`** → **Send app for review** → staged rollout.
5. **Set `PLAY_URL`** on site when public listing URL exists.

### Website

- **Shipped** — showcase, **contact** + **Tester feedback** topic (**`?topic=tester`**), **invite-only internal** QR on **`#get-app`**. No further site work unless owner requests.
...
| Marketing site / internal QR | **`website/index.html`** — **`TESTER_URL`** invite-only QR on **`#get-app`** (family; not closed-test link) |
