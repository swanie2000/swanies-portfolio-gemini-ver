...
**Last updated:** 2026-06-01 — **Live site:** carousel demo (**red Play + bar**, CTA idle/ended); scroll perf (**no header blur**, static bg, native wheel); **no phone-frame hover lift**. Owner verifying on **swaniedesigns.com** after deploy.
...
- **Marketing site (2026-06-01):** **`website/index.html`** — **production** copy; **`#get-app`**; carousel demo (**solid red Play + bar**, CTA idle/ended, ~**13 MB** MP4); **no screenshot hover lift**; scroll perf — **removed sticky header `backdrop-filter`**, **static bg** (no aurora animation), carousel **`overflow-y: hidden`**, no custom wheel handler. Set **`PLAY_URL`** when listing live.
...
1. **Owner:** Verify **https://swaniedesigns.com** — red Play buttons, demo controls, scroll responsiveness, **`#get-app`**.
