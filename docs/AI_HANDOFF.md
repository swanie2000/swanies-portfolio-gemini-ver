...
**Last updated:** 2026-05-16 — **Widget metal row layout fixed (owner verified).** **Win:** Pro home-screen widget — metal assets (e.g. silver bar: **SILVER** / **BAR** / spot price) no longer stretch or overlap; **even dynamic gaps** between rows when widget is resized; full-height widget fits **8** cards. **`PortfolioWidget.kt`:** compact metal type (**9sp** headlines, **7sp** price), uniform **`WidgetAssetCardHeight` = 62dp**, weighted **`Column` + `Spacer(defaultWeight())`** per row (**do not** use **`fillMaxHeight()`** on row wrappers — broke multi-row display). **Next (shipping):** Play internal **`versionCode` 3** AAB + RevenueCat **`goog_…`** in **`local.properties`** — see **Resume** table and **§ Next steps** item **1**.
...
- **Widget (Glance):** Pro **8** / free **3** holdings rows; per-line preference packing + pipe-tolerant parse; list column nests each row (**RemoteViews** direct-child limit); metal widget identity uses **`metalWidgetHeadlinePair`** (3 lines, **9sp** / **7sp**) + uniform **`WidgetAssetCardHeight` (62dp)** for even weighted gaps; **`__METAL_DEFAULT__`** / **`file:`** icon packing on push.
...
| Home screen widget | **`PortfolioWidget.kt`** — **`WidgetAssetLimits`** Pro **8** / free **3**; **`writeWidgetPackedAssetRows`**; **`parseSingleWidgetAssetEntry`**; nested **`Column` + `defaultWeight()`** per row (no **`fillMaxHeight()`** on row wrapper); **`WidgetAssetCardHeight` = 62dp**; metal **`metalWidgetHeadlinePair`** + compact fonts; **`AssetRepository.kt`**, **`SettingsViewModel.kt`**, **`WidgetManagerScreen.kt`**, **`AssetViewModel.kt`**, **`WidgetConfigActivity.kt`**; **`HoldingsUIComponents.kt`** **`metalCardPrimaryLabel`** |
