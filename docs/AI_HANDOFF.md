...
**Last updated:** 2026-05-28 — **Launcher label** **`Portfolio`** under home-screen icon (`launcher_short_name`); Play listing name unchanged. **Repo ready `15 (1.0.15)`** — upload after **14** on device. **Search Console** verified; sitemap submitted. **Testers:** **4 / 12** — **`docs/RECRUIT_INTERNAL_TESTERS.md`**.
...
| **Play internal testing** | **Active: 14 (1.0.14)** on Play; repo **`15 (1.0.15)`** — launcher **Portfolio** label. **Recruiting** (internal cap **100**). |
...
| **Version on device** | Play internal **`v1.0.14 (14)`**; repo **`v1.0.15 (15)`** — short launcher label + prior fixes (**`BuildVersionLabel`** shows version name only). |
...
1. **Play upload:** internal **`15 (1.0.15)`** — launcher **Portfolio** label; run **`.\scripts\verify-aab-revenuecat-key.ps1`** after signed AAB.
2. **Recruit testers (4 → 12):** **`docs/RECRUIT_INTERNAL_TESTERS.md`** — FB profile + crypto groups (not Marketplace); process **`#join-testing`** within 24h.
3. **Search Console:** confirm **Sitemaps** → **`sitemap.xml`** → Success (3 pages); **Performance** after ~1 week.
...
| Play internal ship | **Studio** AAB → **`.\scripts\verify-aab-revenuecat-key.ps1`**; **`14` / `1.0.14`** on Play; repo **`15` / `1.0.15`** (launcher **Portfolio**) ready to upload |
...
- **2026-05-28 — Launcher label Portfolio + 1.0.15:** **`launcher_short_name`** in all locales; manifest **`@string/launcher_short_name`** (was hardcoded **Swanie's Portfolio**). **`versionCode` 15** / **`1.0.15`**. **Handoff + push `main`** → owner uploads internal **15**.
