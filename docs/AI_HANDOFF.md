...
**Last updated:** 2026-06-01 — **Waiting on Google review.** **Closed testing - Alpha:** **Closed Test 23 (1.0.23)** active; **Google Groups** **`testers-community@googlegroups.com`**; TC **opt-in URL** submitted. **Feedback URL:** **`https://swaniedesigns.com/contact.html?topic=tester`** (Play Console saved). **Site:** **`contact.html`** — **Tester feedback** topic (**`4f6197f`**); **`#get-app`** — **invite-only internal** QR for family (**`d64e728`**). **Internal** **23** still live; owner verified **22**/**23** auto-Pro QA. **Gate:** **≥12** opted-in **14+** days — **1** opted-in on closed so far (owner). **No code changes** this handoff pass — docs only.
...
1. **Wait / monitor Google review** (owner) — unblock **production access** or other Console gates as Dashboard shows.
2. **Closed-test gate:** Grow **≥12** opted-in testers on **Closed Alpha** for **14 consecutive days** (owner **1** so far); TC recruitment via submitted opt-in link.
3. **Listing (parallel):** Finish Play **store listing** uploads; **`docs/play_store_long_description_en-US.txt`** → Console.
4. **Console hygiene:** **Dashboard** / **App content** / **Data safety** — nothing blocking when review completes.
5. **Production access:** Apply when review + closed-test metrics pass.
...
1. **Google review** — owner **waiting**; recheck **Dashboard** when status changes.
2. **Closed-test metrics:** **≥12** opted-in, **14+** consecutive days on **Closed Alpha** (recruit via TC; owner opted-in count **1**).
3. **Play listing:** Screenshots, feature graphic, short/long copy; clear remaining **Dashboard** blockers.
4. **Data safety / privacy URLs:** Live **`privacy.html`** anchors in Console if not pasted.
5. **Production access:** Apply when review + closed-test gate pass.
6. **Production release:** **`GRANT_DAYS=0`** build → **`verify-play-release.ps1`** → **Send app for review** → staged rollout.
7. **Set `PLAY_URL`** on site when public listing URL exists.
