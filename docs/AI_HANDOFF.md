...
**Last updated:** 2026-06-01 — **Live site:** carousel demo video — **always-visible native controls**, **CTA overlay** (idle + ended), **2× audio boost** (~**13 MB** H.264 + AAC). Owner verifying on **swaniedesigns.com** after deploy.
...
- **Marketing site (2026-06-01):** **`website/index.html`** — **production** copy; **`#get-app`**; carousel **`final_swanies_portfolio_demo_video_web.mp4`** (~**13 MB**, **720×1280 H.264 + AAC**, **2× audio** via ffmpeg); **native `controls`** always visible (WebKit CSS); **CTA_end_picture** as **poster** + overlay when **idle/ended**; phone frame matches screenshot cards. Master **165 MB** gitignored (**GitHub hard limit 100 MB/file**). Set **`PLAY_URL`** when listing live.
...
1. **Owner:** Verify **https://swaniedesigns.com** — demo video (CTA idle/ended, always-on controls, louder audio), **`#get-app`**.
