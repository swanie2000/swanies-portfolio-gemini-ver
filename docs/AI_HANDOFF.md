...
| Marketing site | `website/` — **`index.html`** (**`#join-testing`** FormSubmit form: name / Play email / message; **`FORMSUBMIT_AJAX_URL`**), **`TESTER_URL`**, **QR**, carousel), **`privacy.html`**, **`press.html`**, **`styles.css`**, **`.github/workflows/deploy-website.yml`** → **`https://swaniedesigns.com`** |
