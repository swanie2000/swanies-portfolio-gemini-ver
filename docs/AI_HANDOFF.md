...
**Last updated:** 2026-05-15 — **Owner workflow:** All Android dev on **one laptop** (no laptop/desktop pair; **`git pull`** is **laptop ↔ GitHub** only). **2026-05-14 ship — Metal + widget headline polish (pushed):** Shared **`metalWidgetHeadlinePair`** in **`HoldingsUIComponents.kt`** — splits on space / **`_`** / camelCase / run-together bullion (**`SILVERBAR`** → **SILVER** / **BAR**). **Collapsed** compact metal title + **expanded** under-icon metal block use **two** single-line **`Text`** rows (no soft-wrap **SILVE**/**R** breaks). **Glance** **`PortfolioWidget`** **`AssetCardOriginal`** uses the same pair (aligned with collapsed list; no extra bare-ticker line vs that row). **Packed widget rows:** **`AssetRepository`** + **`SettingsViewModel`** — **`file:`** / local icon paths **before** metal default sentinel **`__METAL_DEFAULT__`** (custom metal photos win). Widget custom **`file:`** bitmap: neutral icon plate + **crop** fill. Prior **2026-05-13** ship: Pro widget **8** rows, per-line prefs, pipe-safe parse, nested **`Column`** per row (RemoteViews child cap). **`:app:compileDebugKotlin`** green. **Workflow:** **`git pull`** **`origin`** before edits; **`.cursor/rules/git-pull-first.mdc`**.
