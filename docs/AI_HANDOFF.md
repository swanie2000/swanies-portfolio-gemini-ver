...
**Last updated:** 2026-05-11 — **RevenueCat ↔ Google Play credentials (in progress); handoff pushed for laptop close.** You can **close Play / RevenueCat / Cloud browser tabs** — state is captured below and on **GitHub `main`**. **Reopen:** clone or **`git pull`** → read **`docs/AI_HANDOFF.md`** from the top of **§ Current session** through the **Resume** table (same machine or another).

### Resume when you reopen (RevenueCat + Play only)

| Where | State |
|-------|--------|
| **RevenueCat** | App **Swanies Portfolio (Play Store)** · package **`com.swanie.portfolio`** · **Service account JSON uploaded** (`gen-lang-client-0826062826-…json`). Credential validator: **Subscriptions API = OK** · **inappproducts** + **monetization** still **“Credentials need attention”** — **waiting on Google** (can lag **many hours**, RevenueCat docs up to **~24–36 h** after permission saves). **Refresh** in RC after waiting; optional Play **Monetize → Products** → edit description → **Save** (then revert) to nudge propagation. |
| **Google Cloud** | Project **`gen-lang-client-0826062826`** (“Default Gemini Project” in UI). Service account **`RevenueCat Play billing sync`** · email **`revenuecat-play-billing-sync@gen-lang-client-0826062826.iam.gserviceaccount.com`**. JSON key on disk (do **not** commit). Confirm **Google Play Android Developer API** and **Google Play Developer Reporting API** are **Enabled** on this project. |
...
| **When RC shows valid** | RevenueCat → this Play app → **API keys** → copy **`goog_…`** public SDK key → laptop **`local.properties`**: **`REVENUECAT_PUBLIC_API_KEY=`** (never commit). Then **Android Studio** → signed **release AAB** → **Internal testing** new release (**repo** already has **`versionCode` 3** / **`1.0.2`** — Play may still be on **2** until you upload **3**). |
...
- **Testing path:** **Internal testing** release **2 (1.0.1)** on Play + **Swanie's Portfolio Testers**; **`website/index.html`** **`TESTER_URL`**. **Next:** upload **AAB `versionCode` 3** after **`REVENUECAT_PUBLIC_API_KEY`** + RC credentials **valid**. Opt-in link on site.
...
1. **RevenueCat + Play (blocker for store Play builds):** Wait for credential validator to go **valid** (or **~36 h** from last permission **Save**). Confirm Cloud **Reporting API** enabled; RC **refresh**; optional Play **product description** save nudge. When green: copy **`goog_…`** → **`local.properties`** **`REVENUECAT_PUBLIC_API_KEY`** → signed **AAB** → **Internal testing** upload (**`versionCode` 3** in repo vs **2** on Play until uploaded).
...
7. **Pre-launch QA:** Backup round-trip; purchase / restore / expiry; **widgets** (Pro **8**-row) + Pro gates; GRAM/KILO metals on device (Play-delivered build).
