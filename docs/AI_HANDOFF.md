...
**Last updated:** 2026-05-14 — **RevenueCat Play credentials valid; owner paused.** **Next session — do first:** confirm **`REVENUECAT_PUBLIC_API_KEY`** (`goog_…` from RevenueCat) in **`local.properties`** (never commit) → **Android Studio** → **Build → Generate Signed App Bundle** (**release**) → **Play Console → Internal testing** → new release with **`versionCode` 3** AAB → roll out → on a **listed tester** device: install from Play → **subscribe / restore** QA. **Optional later (not blocking):** RevenueCat **Google developer notifications** (Pub/Sub topic + **Connect to Google**). **Reopen:** **`git pull`** → this file from **Current session** down.
